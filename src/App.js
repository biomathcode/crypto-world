...
function CheckBalance() {
  ...
  const [AE, setAE] = useState("ak_RjfYnCacrwT24bSeN8kKuyTd6qCQqY9kibz83HFH97oRf9BJf")  //string
  ...
  function handleSubmit(e) {
    ...
    axios.get('/aeternity/balance/'+ AE,content)
    .then(res => {
      if(res.status === 200) {
        setLoading('success')
        console.log(res)
...
function App() {
  ...
  function handleSubmit(e) {
    ...
    console.log(content)
    axios.put('/ethereum/mint',content)
    .then(res => 
      {
        if(res.status === 200) {
          console.log(res.data)
          setERR('Minting status: '+ res.data.msg + '  status :', res.data.status)
          const burnContent = {
            "fromAddress": AE,
    "amount": amount
          }
          axios.put('/ethereum/burn', burnContent)
          .then(res => {
            if(res.status === 200) {
              console.log(res.data)
              setERR('Burning status :' + res.data.msg + '  status :', res.data.status)
            }else {
...
//Two api calls
  ...
  return (
    ...
    <div className="App" style={{margin: '100px', display:"flex", flex: 'Column', justifyContent: 'space-evenly' }}>
      <div>
      <div>AE to Eth</div>
      <p>{err}</p>
      </div>
